...
export default function ExchangeRatesPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["exchange-rates"] }); toast("تم إضافة سعر الصرف بنجاح", "success"); setDialogOpen(false) },
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["exchange-rates"] }); toast("تم حذف سعر الصرف بنجاح", "success"); setDeleteItem(null) },
    onError: (err: Error) => toast(err.message, "error"),
  })
  ...
  if (error) {
    return <div className="rounded-lg border border-red-200 bg-red-50 p-8 text-center text-red-600">{error instanceof Error ? error.message : "خطأ في تحميل البيانات"}</div>
  }

  return (
    <div className="space-y-6">
      <PageHeader title="سعر الصرف" description="إدارة أسعار صرف الدولار مقابل الدينار" actionLabel="إضافة سعر صرف" onAction={() => setDialogOpen(true)} />
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border">
                  ...
                  <th className="text-center p-4 text-sm font-medium text-gray-500 w-20">حذف</th>
                </tr>
              </thead>
              <tbody>
                {rates.map((rate, idx) => (
                  <tr key={rate.id} className="border-b border-border hover:bg-muted/30">
                    <td className="p-4 text-sm text-gray-600">{formatDate(rate.date)}</td>
                    <td className="p-4">
                      <span className="font-medium">1 USD = {formatCurrency(rate.usdToIqd)} IQD</span>
                      {idx === 0 && <Badge variant="success" className="mr-2">الحالي</Badge>}
                    </td>
                    <td className="p-4 text-sm text-gray-600">{rate.note || "-"}</td>
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} title="إضافة سعر صرف">
        <ExchangeRateForm onSubmit={handleSubmit} loading={createMutation.isPending} />
      </Dialog>
