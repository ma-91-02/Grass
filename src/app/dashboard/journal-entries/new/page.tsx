...
export default function NewJournalEntryPage() {
  ...
  const { data: allAccounts = [], isLoading: accountsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const params = new URLSearchParams({ companyId: userCompanyId });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const json = await res.json();
      if (!json.success) {
        throw new Error(json.error || "فشل إنشاء القيد");
      }
      toast("تم إنشاء القيد بنجاح", "success");
      router.push("/dashboard/journal-entries");
    } catch (err) {
      toast(
        err instanceof Error ? err.message : "فشل إنشاء القيد",
        "error",
      );
    } finally {
      setSubmitting(false);
