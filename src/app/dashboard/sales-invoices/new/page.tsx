...
export default function NewSalesInvoicePage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const json = await res.json();
      if (!json.success) {
        throw new Error(json.error || "فشل إنشاء الفاتورة");
      }
      toast("تم إنشاء الفاتورة بنجاح", "success");
      router.push(`/dashboard/sales-invoices/${json.data.id}`);
    } catch (err) {
      toast(err instanceof Error ? err.message : "فشل إنشاء الفاتورة", "error");
    } finally {
      setSubmitting(false);
    }
  };
